...
if __name__ == "__main__":
    ...
    video_writer = cv2.VideoWriter(
        "debug/test_joints.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 50, (frame_width, frame_height)
    )

    joints_schedule = np.array([
        [200, 50, 5, 1],
        [150, 25, 3, 1],
        [250, 80, 10, 5],
        [100, 20, 3, 0.5],
    ...
    while True:
            if it % 2000 == 0:
                if it // 2000 >= len(joints_schedule):
                    break
                else:
                    s1, s2, d1, d2 = joints_schedule[it // 2000]
                    stiffness = {"Hip": s1, "Knee": s1, "Ankle": s2} # [N*m/rad]
                    damping = {"Hip": d1, "Knee": d1, "Ankle": d2} # [N*m*s/rad]
                    for i in range(mj_model.nu):
            ...
            if it % 10 == 0:
                ...
                cv2.putText(
                    frame_bgr,
                    f"Stiffness: Hip={stiffness['Hip']}, Knee={stiffness['Knee']}, Ankle={stiffness['Ankle']}",
                    (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.8, (0, 255, 0), 2, cv2.LINE_AA)  # Green text
                cv2.putText(
                    frame_bgr,
                    f"Damping: Hip={damping['Hip']}, Knee={damping['Knee']}, Ankle={damping['Ankle']}",
                    (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.8, (0, 255, 0), 2, cv2.LINE_AA)  # Green text
                video_writer.write(frame_bgr)

    video_writer.release()
    print("Video saved to 'debug/test_joints.mp4'")
    print("Done.")