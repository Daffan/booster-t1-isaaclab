...
if __name__ == "__main__":
    ...
    video_writer = cv2.VideoWriter(
        "debug/test_omni.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 50, (frame_width, frame_height)
    )
    ...
    while True:
            if it % 2000 == 0:
                if it // 2000 >= len(command_schedule):
                    break
                else:
                    lin_vel_x, lin_vel_y, ang_vel_yaw = command_schedule[it // 2000]
            ...
            if it % 10 == 0:
                ...
                # write current true velocity
                cv2.putText(frame_bgr, f"True Vel: x={base_lin_vel[0]:.2f}, y={base_lin_vel[1]:.2f}, yaw={base_ang_vel[2]:.2f}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.8, (0, 255, 0), 2, cv2.LINE_AA)
                video_writer.write(frame_bgr)
                video_writer.write(frame_bgr)

    video_writer.release()
    print("Video saved to 'debug/test_omni.mp4'")
    print("Done.")