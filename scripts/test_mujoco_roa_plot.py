...
if __name__ == "__main__":
    ...
    lab2gym = [0, 6, 1, 7, 2, 8, 3, 9, 4, 10, 5, 11] # for isaac lab to gym env

    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    save_path = os.path.join("debug", timestamp)
    os.makedirs(save_path, exist_ok=True)

    frame_width = 640
    frame_height = 480
    video_writer = cv2.VideoWriter(
        os.path.join(save_path, "test_plot.mp4"), cv2.VideoWriter_fourcc(*"mp4v"), 50, (frame_width, frame_height)
    )

    command_schedule = np.array([
        [0.0, 0.0, 0.0],
        [0.5, 0.0, 0.0],
        [1.0, 0.5, 0.0],
        # [0.0, 0.5, 0.0],
    ...
    stiffness = {"Hip": 200.0, "Knee": 200.0, "Ankle": 50} # [N*m/rad]
    damping = {"Hip": 5.0, "Knee": 5.0, "Ankle": 1} # [N*m*s/rad]
    for i in range(mj_model.nu):
        for name in stiffness.keys():
            if name in mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i):
                dof_stiffness[i] = stiffness[name]
                dof_damping[i] = damping[name]
    print(f"Updated joint stiffness to: {dof_stiffness}, damping to: {dof_damping}")
    ...
    while True:
            if it % duration == 0:
                if it // duration >= len(command_schedule):
                    break
                else:
                    lin_vel_x, lin_vel_y, ang_vel_yaw = command_schedule[it // duration]
            ...
            if (it) % cfg["control"]["decimation"] == 0:
                ...
                # get the contact force of the two feet with the ground
                # import ipdb; ipdb.set_trace()
                # print(np.isclose(mj_data.cfrc_ext, 0.0).all())
                # print(mj_data.cfrc_ext.shape)
                left_force = mj_data.cfrc_ext[mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_SITE, "left_foot_link")][[2]]
                right_force = mj_data.cfrc_ext[mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_SITE, "right_foot_link")][[2]]

                if len(obss) == 0:
                    # obss.extend([np.zeros_like(obs)] * history) 
                    obss.extend([obs.copy()] * history)
                obss.append(obs)  # append the current frame
                # import ipdb; ipdb.set_trace()
                obs_hist.append(np.concatenate([obs.copy(), left_force, right_force], axis=0))
            ...
            if it % 10 == 0:
                ...
                cv2.putText(frame_bgr, f"Command: x={lin_vel_x}, y={lin_vel_y}, yaw={ang_vel_yaw}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.8, (0, 255, 0), 2, cv2.LINE_AA)  # Green text
                video_writer.write(frame_bgr)

    # close the mujoco simulation
    video_writer.release()
    print(f"Video saved to {os.path.join(save_path, 'test_plot.mp4')}")
    print("Done.")

    # save obs_hist into file
    obs_hist = np.array(obs_hist)
    np.save("debug/obs_hist.npy", obs_hist)
    print(f"Obs history saved to debug/obs_hist.npy")
    ...
    for i in range(11):
        axes[i // 4, i % 4].plot(obs_hist[:, i])
    axes[2, 3].plot(obs_hist[:, -1])  # this is the force
    plt.savefig(os.path.join(save_path, "obs1.png"))
    plt.close()

    # plot the obs2
    fig, axes = plt.subplots(3, 4, figsize=(18, 10))
    for i in range(12):
        axes[i // 4, i % 4].plot(obs_hist[:, i + 11])
        axes[i // 4, i % 4].set_title(f"{mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i)}")
    plt.savefig(os.path.join(save_path, "obs2.png"))
    plt.close()

    # plot the obs3
    fig, axes = plt.subplots(3, 4, figsize=(18, 10))
    for i in range(12):
        axes[i // 4, i % 4].plot(obs_hist[:, i + 23])
        axes[i // 4, i % 4].set_title(f"{mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i)}")
    plt.savefig(os.path.join(save_path, "obs3.png"))
    plt.close()

    # plot the obs4
    fig, axes = plt.subplots(3, 4, figsize=(18, 10))
    for i in range(12):
        axes[i // 4, i % 4].plot(obs_hist[:, i + 35])
        axes[i // 4, i % 4].set_title(f"{mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i)}")
    plt.savefig(os.path.join(save_path, "obs4.png"))
    plt.close()