...
if __name__ == "__main__":
    ...
    gait_frequency = gait_process = 2.0
    lin_vel_x = 1.0
    ...
    video_writer = cv2.VideoWriter(
        "debug/test_push.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 50, (frame_width, frame_height)
    )

    push_schedule = np.array([
        [0.1, 0.0, 0.0],
        [0.2, 0.0, 0.0],
        [0.4, 0.0, 0.0],
        [0.5, 0.5, 0.0],
    ...
    while True:
            if it % 2000 == 0:
                if it // 2000 >= len(push_schedule):
                    break
                else:
                    fx, fy, fz = push_schedule[it // 2000]
                    mj_data.qvel[:6] += np.array([fx, fy, fz, 0.0, 0.0, 0.0])
                    print(f"Pushing with velocity: x={fx}, y={fy}, z={fz}")
            ...
            if it % 10 == 0:
                ...
                cv2.putText(
                    frame_bgr,
                    f"push: x={fx}, y={fy}, z={fz}",
                    (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.8, (0, 255, 0), 2, cv2.LINE_AA)  # Green text
                video_writer.write(frame_bgr)

    video_writer.release()
    print("Video saved to 'debug/test_push.mp4'")
    print("Done.")